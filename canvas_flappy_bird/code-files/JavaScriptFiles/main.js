...
window.addEventListener('load', function(){
    ...
    class Game{
        ...
        constructor(width, height){
            ...
            this.pipes = [];
            //this.pipes = [new Pipe(this, 0, 0, true)]; new Pipe( this, this.width - 200,  Math.random() * ( this.height * (1/2) ) + this.height * (1/3), false )
            this.players = [new Bird(this, this.width / 10, this.height / 5)];//just 1 player for now though XD.
            this.background = new BackGround(this);
            this.iu = new UI(this);
            this.inputs = new Inputs();

            //all players share a x pos
            this.addPointPosition = this.players[0].x + this.players[0].width / 2;
        }

        addPipes(){
            let randomPos = Math.random() * ( this.height / 2 ) + this.height / 3;
            const lastAddedBottomPipe = this.pipes[ this.pipes.length - 1 ];
            //we can't have one pipe gap at the top and next one at the bottom or player cant get to it. 
        ...
        restartGame(){
            ...
            this.pipes = [];
            this.isGameover = false;
            this.players[0].x = this.width / 10;// = [new Bird(this, this.width / 10, this.height / 5)];
            this.players[0].y  = this.height / 5;
            this.players[0].jump();
            this.background = new BackGround(this);
        }
        ...
        update(deltatime){
            ...
            this.players.forEach(player => player.update(deltatime, this.pipes, this.inputs.keys));
